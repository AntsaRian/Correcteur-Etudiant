...
export default function StatutList() {
    ...
    useEffect(() => {
        fetch(`${API}/statuts`)
            .then(r => r.json())
            .then(data => { setStatuts(data); setLoading(false); })
    ...
    const handleDelete = async (id) => {
        ...
        setDeletingId(id);
        await fetch(`${API}/statuts/${id}`, { method: "DELETE" });
        setStatuts(prev => prev.filter(d => d.id !== id));
        setDeletingId(null);
    };
        ...
        <div className="etu-page">
            <div className="etu-header">
                <div className="etu-header-left">
                    ...
                    <h1 className="etu-title">Statuts</h1>
                </div>
                <button className="btn-primary" onClick={() => navigate("/Statut/create")}>
                    + Nouveau statut
                </button>
            </div>
                            ...
                            {statuts.length === 0 ? (
                                ...
                                <tr key={d.id} className={i % 2 === 0 ? "row-even" : "row-odd"}>
                                    ...
                                    <td className="cell-actions">
                                        <button
                                            className="btn-edit"
                                            onClick={() => navigate(`/Statut/edit/${d.id}`)}
                                        >
                                            Modifier
                                        </button>
                                        <button
