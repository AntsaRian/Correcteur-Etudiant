...
export default function ParametreEdit() {
    const { id } = useParams();
    ...
    useEffect(() => {
        Promise.all([
            ...
            fetch(`${API}/parametres/${id}`).then(r => r.json()),
        ]).then(([m, o, r, p]) => {
            setMatieres(m);
            setOperateurs(o);
            setResolutions(r);
            setForm({
                id_matiere: p.matiere?.id || "",
                diff: p.diff || "",
                id_operateur: p.operateur?.id || "",
                id_resolution: p.resolution?.id || "",
            });
            setFetching(false);
        });
    }, [id]);
    ...
    const handleSubmit = async () => {
        ...
        const res = await fetch(`${API}/parametres/${id}`, {
            method: "PUT",
    ...
    if (fetching) return <div className="param-page"><div className="param-loading">Chargement…</div></div>;

    return (
        <div className="param-page">
            <div className="param-header">
                <div className="param-header-left">
                    <span className="param-label">MODIFICATION · #{id}</span>
                    <h1 className="param-title">Éditer le paramètre</h1>
                </div>
