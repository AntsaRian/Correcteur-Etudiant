...
(3, 4, 3, 1),  -- Maths: diff=3.0, operateur >, resolution plus grand
(3, 1, 3, 3),  -- Francais: diff=2.0, operateur <, resolution plus petit
...
(3, 14.5, 1, 1),  -- Maths par correcteur 1
(3, 13.0, 2, 1);  -- Maths par correcteur 2
...
-- Sujet vaovao
-- Forage rano
-- 1. Miteny besoins  (Fokotany ...) => devis etude => manaiky (mandoa vola) => etude
-- 2. Etude (aiza no mora azoana rano) => (atao / tsy atao) => devis forage => mivaly ny etude (realiser / ts realiser)
-- 3. Forage:
-- ETAPE 3: => misy status we tamzao en cours, tamzao vita
-- nanomboka asa (date debut - date fin)
-- nahazo rano (date debut - date fin)
-- test sanitaire (eau potable) (date debut - date fin)